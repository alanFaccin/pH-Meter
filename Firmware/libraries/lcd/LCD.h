...
//Modified version By Benato Denis & Davide Bortolami

//This library allows to easly write on a standard LCD based on an HD44780 controller
...
//just copy the following define and
...
#define activate_printf

//How to use:
//-Include the library
//-LCD_Init(); once
//-Move to a position with LCD_Set_Cursor(Line, Column);
//-Write something with LCD_printf("num: %d char: %c", 1, 'A');
//-Write a char with LCD_Write_Char('a');
//-Clear the LCD with LCD_Clear();
//-if you want, write a custom char in a position from 1 to 7
//-Save a custom char with LCD_Custom_Char(array, position) with position from 1 to 7
//-Write the custom char with LCD_Write_Char(position)
//
//
//use this tool to create the array for the custom char http://www.quinapalus.com/hd44780udg.html
