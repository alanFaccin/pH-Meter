#ifndef __I2C_SEVERI__
#define __I2C_SEVERI__


//***********************************************************
//Tested and modified by Davide Bortolami
//
//piclib.ddavidebor.it ddavidebor@gmail.com
//
//during year 2015
/*
 DESCRIPTION:
...
//------------------------------------------------------------------
void I2C_Delay(void);				//Adjust as needed
void SCL_Low(void);				//Asserts SCL low
void SCL_High(void);				//Releases SCL to float high
void SDA_Low(void);				//Asserts SDA low
void SDA_High(void);				//Releases SDA to float high
void I2C_SendZero(void);			//Sends a "zero" bit
void I2C_SendOne(void);				//Sends a "one" bit
void I2C_Start(void);				//Sends the I2C start sequence
void I2C_Stop(void);				//Sends the I2C stop sequence
void I2C_WaitForSlave(void);			//Waits for SCL to be released
void I2C_Ack(void);
...
void I2C_Ack(void)
{
    SDA_Low();        //Pull SDA low to indicate Positive ACK

    //generate a clock pulse
    __delay_us(1);
    SCL_High();       // Wait for Some time and Pull the SCL line High
    __delay_us(1);        // Wait for Some time
    SCL_Low();;        // Pull back the SCL line low to Generate a clock pulse

    SDA_High();        // Pull SDA back to High(IDLE state)
}
...
void I2C_WaitForSlave(){
	...
	//while(SCL_PORT & SCL_ON_MASK == 0x00);	//Loop while the SCL line is low
        while(SDA_PORT & SDA_ON_MASK == 0x00);
        #endif
	}	
